<?php
$page_title = 'Reviews - Macoy\'s Straightening Beauty Salon';
			...
			<main>
                ...
                <div class="reviews-container">
                    ...
                    <div class="overall-feedback">
                        ...
                        <p>Share your general experience with us</p>
                        <button class="overall-feedback-btn" onclick="openFeedbackModal()">Submit Overall Feedback</button>
                    </div>

                    <!-- Reviews List -->
                    <div class="reviews-list" id="reviewsList">
                        <?php
                        $reviews_data = json_decode(file_get_contents('data/reviews.json'), true) ?? [];
                        $user_reviews = array_filter($reviews_data, fn($r) => $r['user_id'] == ($_SESSION['id'] ?? 0));
                        foreach ($user_reviews as $review):
                        ?>
                        <div class="review-item submitted" data-booking-id="<?php echo $review['booking_id']; ?>">
                            <div class="review-header">
                                <div class="review-details">
                                    <h4><?php echo htmlspecialchars($review['service']); ?></h4>
                                    <div class="review-meta">
                                        <?php echo date('M d, Y', strtotime($review['date'])); ?> • <?php echo htmlspecialchars($review['stylist']); ?>
                                    </div>
                                </div>
                    ...
                    <!-- Empty State -->
                    <div class="empty-reviews" id="emptyReviews" <?php if (!empty($user_reviews)): ?>style="display: none;"<?php endif; ?>>
                        <i class='bx bx-star'></i>
                        <h3>No Bookings to Review</h3>
                        <p>Complete some bookings first! Check <a href="mybookings.php#history">My Bookings History</a></p>
                    </div>
                </div>
			</main>
			<!-- beforefooter  -->  
...
<!-- Reviews JS with PHP data -->
<script>
    // Use PHP data in JS
    const bookingHistory = <?php echo json_encode($bookingHistory ?? []); ?>;
    // ... rest JS from original
</script>

<!-- Overall Feedback Modal -->
<!-- Same modal HTML as original -->

<script>
    // Mock booking history data (from PHP or data/reviews.json)
    // Load saved reviews etc.
</script>
